...
def run(
    ...
    # 判断输入源类型
    is_file = Path(source).suffix[1:] in (IMG_FORMATS + VID_FORMATS)  # 是否是文件
    is_url = source.lower().startswith(('rtsp://', 'rtmp://', 'http://', 'https://'))  # 是否是URL
    webcam = source.isnumeric() or source.endswith('.txt') or (is_url and not is_file)  # 是否是摄像头/文本流
    screenshot = source.lower().startswith('screen')  # 是否截屏输入
    if is_url and is_file:
        source = check_file(source)  # 如果是URL文件则下载
    ...
    model = DetectMultiBackend(weights, device=device, dnn=dnn, data=data, fp16=half)  # 初始化多后端模型
    stride, names, pt = model.stride, model.names, model.pt  # 获取模型的步长、类别名称和是否PyTorch模型
    ...
    if webcam:  # 摄像头/视频流模式
        ...
        dataset = LoadStreams(source, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)  # 视频流加载器
        bs = len(dataset)  # 批处理大小等于视频源数量
    elif screenshot:  # 屏幕截图模式
        dataset = LoadScreenshots(source, img_size=imgsz, stride=stride, auto=pt)  # 屏幕截图加载器
    else:  # 图像/视频文件模式
    ...
    # 模型预热（用虚拟数据运行一次推理）
    model.warmup(imgsz=(1 if pt or model.triton else bs, 3, *imgsz))  # 根据后端类型调整输入形状
    seen, windows, dt = 0, [], (Profile(), Profile(), Profile())  # 初始化计数器、窗口和时间分析器

    # 遍历数据集的每一帧
    for path, im, im0s, vid_cap, s in dataset:
        with dt[0]:  # 预处理时间统计
            im = torch.from_numpy(im).to(model.device)  # 将图像转换为PyTorch张量并送到设备
            im = im.half() if model.fp16 else im.float()  # 转换为半精度或单精度
            im /= 255  # 归一化像素值到0-1
            if len(im.shape) == 3:
        ...
        with dt[1]:  # 推理时间统计
            ...
            pred = model(im, augment=augment, visualize=visualize)  # 执行模型推理
        ...
        for i, det in enumerate(pred):  # 遍历每张图像的检测结果
            seen += 1  # 已处理图像计数器
            # 获取原始图像和路径信息
            if webcam:  # 批处理模式（摄像头/视频流）
                p, im0, frame = path[i], im0s[i].copy(), dataset.count  # 当前路径、图像、帧数
                s += f'{i}: '  # 信息字符串
            else:  # 单图像模式
            ...
            txt_path = str(save_dir / 'labels' / p.stem) + ('' if dataset.mode == 'image' else f'_{frame}')  # 标签路径
            s += '%gx%g ' % im.shape[2:]  # 添加图像尺寸到信息字符串
            gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]  # 归一化系数（宽高宽高）
            imc = im0.copy() if save_crop else im0  # 复制图像用于裁剪保存
            annotator = Annotator(im0, line_width=line_thickness, example=str(names))  # 创建标注器对象
            ...
            if len(det):  # 如果有检测结果
                ...
                for c in det[:, 5].unique():
                    ...
                    s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "  # 添加到信息字符串

                # 处理每个检测结果
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # 写入文本文件
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # 转换为归一化xywh格式
                        line = (cls, *xywh, conf) if save_conf else (cls, *xywh)  # 构造行数据
                    ...
                    if save_img or save_crop or view_img:
                        c = int(cls)  # 类别索引
                        label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')  # 构造标签
                        annotator.box_label(xyxy, label, color=colors(c, True))  # 绘制框和标签
                    if save_crop:  # 保存裁剪的检测框
                        save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)

            # 显示结果
            im0 = annotator.result()  # 获取绘制后的图像
            if view_img:
                if platform.system() == 'Linux' and p not in windows:  # Linux下窗口设置
                    windows.append(p)
                    cv2.namedWindow(str(p), cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)  # 允许调整窗口大小
                    cv2.resizeWindow(str(p), im0.shape[1], im0.shape[0])
                cv2.imshow(str(p), im0)  # 显示图像
                cv2.waitKey(1)  # 等待1ms（保持显示）

            # 保存结果（图像/视频）
            if save_img:
                if dataset.mode == 'image':  # 图像模式直接保存
                    cv2.imwrite(save_path, im0)
                else:  # 视频或流模式
                    if vid_path[i] != save_path:  # 新视频需要初始化写入器
                        vid_path[i] = save_path
                        if isinstance(vid_writer[i], cv2.VideoWriter):
                            vid_writer[i].release()  # 释放之前的写入器
                        # 获取视频参数
                        if vid_cap:  # 视频文件
    ...
    if update:  # 更新模型权重（修复SourceChangeWarning）
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco128.yaml', help='数据集配置文件')
